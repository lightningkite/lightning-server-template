...
resource "aws_s3_bucket" "files" {
  bucket_prefix = "lktemplate-templateapi-files"
...
resource "aws_iam_policy" "files" {
  name        = "LKTemplate-templateapi-files"
  path = "/lktemplate/templateapi/files/"
  description = "Access to the LKTemplate-templateapi_files bucket"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
