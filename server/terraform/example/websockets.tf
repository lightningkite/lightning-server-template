...
resource "aws_cloudwatch_log_group" "ws_api" {
  name = "LKTemplate-ws-gateway-log"
...
resource "aws_iam_policy" "api_gateway_ws" {
  name        = "LKTemplate-api_gateway_ws"
  path = "/lktemplate/api_gateway_ws/"
  description = "Access to the LKTemplate_api_gateway_ws management"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
