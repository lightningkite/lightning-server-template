...
object Server : ServerPathGroup(ServerPath.root) {
    ...
    val webUrl = setting("webUrl", default = "lightningtemplate:///session")
    ...
    val index = get.handler {
        ...
        val url = "${webUrl()}?jwt=$token&server=${URLEncoder.encode(generalSettings().publicUrl, Charsets.UTF_8)}"
        if(webUrl().startsWith("http"))
            return@handler HttpResponse.redirectToGet(url)
